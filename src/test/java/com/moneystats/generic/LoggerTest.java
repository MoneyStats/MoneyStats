...
public class LoggerTest {
  ...
  public void logTimeTrackerFailure() {
    LogTimeTracker tracker = LogTimeTracker.startInvocation(LogTimeTracker.ActionType.APP_SERVICE_LOGIC, "", "", "");
  ...
  @Test
  public void checkAllServicesAreWellAnnotatedForLogs()
      throws Exception {
    List<Class> allClasses = Arrays.asList(getClasses("com.moneystats"));
    ...
    List<Method> notWellAnnotated =
        allPublicMethods.stream()
            .filter(m -> m.getAnnotation(LoggerMethod.class) == null)
            .collect(Collectors.toList());
